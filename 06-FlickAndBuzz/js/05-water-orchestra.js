...
window.addEventListener("DOMContentLoaded", function() {
  ...
  new five.Board(/*{port: "COM6"}*/).on("ready", function() {
    ...
    var keypad = new five.Keypad({
      ...
      // sensitivity: [
      //   {press: 0.95, release: 0.975},
      //   {press: 0.98, release: 0.99},
      //   {press: 0.95, release: 0.975},
      //   {press: 0.95, release: 0.975},
      //   {press: 0.95, release: 0.975},
      // ]
    ...
    keypad.on("press", function(data) {
      ...
      data.which.forEach(function(index) {
        ...
        if (index < notes.length) synth.triggerAttack(notes[index]);
      });
    ...
    keypad.on("release", function(data) {
      ...
      data.which.forEach(function(index) {
        ...
        if (index < notes.length) synth.triggerRelease(notes[index]);
      });
